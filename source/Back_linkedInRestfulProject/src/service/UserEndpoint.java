...
public class UserEndpoint {
	...
	public Response findById(@PathParam("id") final Integer id) throws IOException {
		...
		if (userd != null) {
			...
			user.setPhotoBean(createPhotoBean(id));
		}
		if (user == null) {
	...
	public Response updateUser(UserBean user) {
		...
		oldUserd.setIdUser(user.getIdUser());
		oldUserd.setLastName(user.getLastName());
		oldUserd.setFirstName(user.getFirstName());
		oldUserd.setPassword(user.getPassword());
		oldUserd.setEmail(user.getEmail());
		oldUserd.setPhoneNumber(user.getPhoneNumber());
		oldUserd.setIsModerator(0);
		oldUserd.setAdvertisments(oldUserd.getAdvertisments());
		oldUserd.setArticles(oldUserd.getArticles());
		oldUserd.setConnections(oldUserd.getConnections());
		oldUserd.setConnectionRequests(oldUserd.getConnectionRequests());
		oldUserd.setConversations(oldUserd.getConversations());
		oldUserd.setSkills(oldUserd.getSkills());
		
		oldUserd.setEducationText(user.getEducationText());
		oldUserd.setJobExperienceText(user.getJobExperienceText());
		oldUserd.setIsPublicEducation(user.getIsPublicEducation());
		oldUserd.setIsPublicJob(user.getIsPublicJob());
		oldUserd.setIsPublicSkill(user.getIsPublicSkill());
		//userDao.updateUser(userd);
		userDao.mergeUser(oldUserd);
	...
	@Path("/users")
    @Produces({ "application/json" })
	public Response returnUsers() throws IOException {
		UserDB userDao = new UserDB();
	...
	@Produces({"application/json"})
	@Path("/search")
	public Response search(SearchBean searchBean) throws IOException {
	...
	public UserBean createUserBeanFromEntity(entities.User userd) throws IOException {
		...
		if (userd != null) {
			...
			user.setIsPublicJob(userd.getIsPublicJob());
			user.setIsPublicSkill(userd.getIsPublicSkill());
			user.setPhotoBean(createPhotoBean(userd.getIdUser()));
		}
		else {
			user.setIdUser(-1);
		}
	...
	@Consumes({"application/json"})
	public Response updatePhoto(final PhotoBean photoBean) throws IOException {

		UserDB userDao = new UserDB();
		entities.User userd = userDao.getById(photoBean.getUserId());
		
		if((userd.getPhotoUrl() != null) && (!userd.getPhotoUrl().equals("NO")) && (!userd.getPhotoUrl().equals("EMPTY"))) {
			File file = new File(userd.getPhotoUrl());
			file.delete();
		}
		byte[] valueDecoded = Base64.decodeBase64(photoBean.getPhoto());

		
		String path = "/home/mike/Desktop/linkedInFileSystem/userPics/user" + photoBean.getUserId() + "." + photoBean.getFileType();
		FileOutputStream outputStream = new FileOutputStream(path);
		outputStream.write(valueDecoded);
		outputStream.flush();
		outputStream.close();
		
		userd.setPhotoUrl(path);
	...
	@Path("/profPic/{id:[0-9]*}")
	@Produces({"application/json"})
	public Response getProfPic(@PathParam("id") int id) throws IOException {
		UserDB userDao = new UserDB();
		entities.User userd = userDao.getById(id);
		PhotoBean prof = new PhotoBean();
		if((userd.getPhotoUrl() != null) && (!userd.getPhotoUrl().equals("NO")) && (!userd.getPhotoUrl().equals("EMPTY")))
		{
		    ...
		    prof.setPhoto(new String(encoded));
		    prof.setUserId(id);
		    prof.setFileType(getFileExtension(file));
	...
	private static PhotoBean createPhotoBean(int id) throws IOException {
		UserDB userDao = new UserDB();
		entities.User userd = userDao.getById(id);
		PhotoBean prof = new PhotoBean();
		if((userd.getPhotoUrl() != null) && (!userd.getPhotoUrl().equals("NO")) && (!userd.getPhotoUrl().equals("EMPTY")))

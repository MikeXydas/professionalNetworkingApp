...
public class ArticleEndpoint {
	...
	@Path("/postComment")
	@Consumes({"application/json"})
	public Response postComment(final PostCommentBean commBean) {
		ArticleDB articleDao = new ArticleDB();
	...
	private ArticleBean createArticleBean(entities.Article articled) throws IOException {
		...
		for(int i = 0; i < commentsd.size(); i++) {
			...
			temp.setFirstName(tempCommUser.getFirstName());
			temp.setLastName(tempCommUser.getLastName());
			temp.setPhotoBean(createPhotoBean(tempCommUser.getIdUser()));
			comments.add(temp);
		}
	...
	@Produces({"application/json"})
	public Response returnNotifications(@PathParam("id") final Integer id) throws IOException {
		UserDB userDao = new UserDB();
		...
		if(interestsd != null) {
			for(int whichInt = 0; whichInt < interestsd.size(); whichInt++) {
				...
				temp.setPhotoBean(createPhotoBean(userd.getIdUser()));
